    ...
    fn from_str(s: &str) -> std::result::Result<Self, Self::Err> {
        ...
        while key_result.is_some() {
            let val = match string_iter.next() {
                ...
                None => {
                    return std::result::Result::Err(
                        "Missing value while parsing ArenaLeaderboardEntry".to_string(),
                    );
                }
            };
            ...
            if let Some(key) = key_result {
                ...
                match key {
                    ...
                    _ => {
                        return std::result::Result::Err(
                            "Unexpected key while parsing ArenaLeaderboardEntry".to_string(),
                        );
                    }
                }
            }
        ...
        std::result::Result::Ok(ArenaLeaderboardEntry {
            rank: intermediate_rep
                ...
                .into_iter()
                .next()
                .ok_or_else(|| "rank missing in ArenaLeaderboardEntry".to_string())?,
            average_placement: intermediate_rep
                .average_placement
                .into_iter()
                .next()
                .ok_or_else(|| "averagePlacement missing in ArenaLeaderboardEntry".to_string())?,
            last_updated: intermediate_rep
                .last_updated
                .into_iter()
                .next()
                .ok_or_else(|| "lastUpdated missing in ArenaLeaderboardEntry".to_string())?,
            matches_played: intermediate_rep
                .matches_played
                .into_iter()
                .next()
                .ok_or_else(|| "matchesPlayed missing in ArenaLeaderboardEntry".to_string())?,
            player_name: intermediate_rep
                .player_name
                .into_iter()
                .next()
                .ok_or_else(|| "playerName missing in ArenaLeaderboardEntry".to_string())?,
            region: intermediate_rep
                .region
                .into_iter()
                .next()
                .ok_or_else(|| "region missing in ArenaLeaderboardEntry".to_string())?,
            tag_line: intermediate_rep
                .tag_line
                .into_iter()
                .next()
                .ok_or_else(|| "tagLine missing in ArenaLeaderboardEntry".to_string())?,
            top_four_rate: intermediate_rep
                .top_four_rate
                .into_iter()
                .next()
                .ok_or_else(|| "topFourRate missing in ArenaLeaderboardEntry".to_string())?,
            win_rate: intermediate_rep
                .win_rate
                .into_iter()
                .next()
                .ok_or_else(|| "winRate missing in ArenaLeaderboardEntry".to_string())?,
        })
    }
}
...
impl std::convert::TryFrom<header::IntoHeaderValue<ArenaLeaderboardEntry>> for HeaderValue {
    ...
    fn try_from(
        hdr_value: header::IntoHeaderValue<ArenaLeaderboardEntry>,
    ) -> std::result::Result<Self, Self::Error> {
        let hdr_value = hdr_value.to_string();
        match HeaderValue::from_str(&hdr_value) {
            std::result::Result::Ok(value) => std::result::Result::Ok(value),
            std::result::Result::Err(e) => std::result::Result::Err(format!(
                r#"Invalid header value for ArenaLeaderboardEntry - value: {hdr_value} is invalid {e}"#
            )),
        }
    }
}
            ...
            std::result::Result::Ok(value) => {
                match <ArenaLeaderboardEntry as std::str::FromStr>::from_str(value) {
                    ...
                    std::result::Result::Err(err) => std::result::Result::Err(format!(
                        r#"Unable to convert header value '{value}' into ArenaLeaderboardEntry - {err}"#
                    )),
                }
            }
            std::result::Result::Err(e) => std::result::Result::Err(format!(
...
impl ArenaLeaderboardResponse {
    #[allow(clippy::new_without_default, clippy::too_many_arguments)]
    pub fn new(
        entries: Vec<models::ArenaLeaderboardEntry>,
        page: u32,
        page_size: u32,
        sort_by: String,
        sort_order: String,
    ...
    fn from_str(s: &str) -> std::result::Result<Self, Self::Err> {
        ...
        while key_result.is_some() {
            let val = match string_iter.next() {
                ...
                None => {
                    return std::result::Result::Err(
                        "Missing value while parsing ArenaLeaderboardResponse".to_string(),
                    );
                }
            };
        ...
        std::result::Result::Ok(ArenaLeaderboardResponse {
            entries: intermediate_rep
                ...
                .into_iter()
                .next()
                .ok_or_else(|| "entries missing in ArenaLeaderboardResponse".to_string())?,
            page: intermediate_rep
                .page
                .into_iter()
                .next()
                .ok_or_else(|| "page missing in ArenaLeaderboardResponse".to_string())?,
            page_size: intermediate_rep
                .page_size
                .into_iter()
                .next()
                .ok_or_else(|| "pageSize missing in ArenaLeaderboardResponse".to_string())?,
            sort_by: intermediate_rep
                .sort_by
                .into_iter()
                .next()
                .ok_or_else(|| "sortBy missing in ArenaLeaderboardResponse".to_string())?,
            sort_order: intermediate_rep
                .sort_order
                .into_iter()
                .next()
                .ok_or_else(|| "sortOrder missing in ArenaLeaderboardResponse".to_string())?,
            total_items: intermediate_rep
                .total_items
                .into_iter()
                .next()
                .ok_or_else(|| "totalItems missing in ArenaLeaderboardResponse".to_string())?,
            total_pages: intermediate_rep
                .total_pages
                .into_iter()
                .next()
                .ok_or_else(|| "totalPages missing in ArenaLeaderboardResponse".to_string())?,
        })
    }
}
...
impl std::convert::TryFrom<header::IntoHeaderValue<ArenaLeaderboardResponse>> for HeaderValue {
    ...
    fn try_from(
        hdr_value: header::IntoHeaderValue<ArenaLeaderboardResponse>,
    ) -> std::result::Result<Self, Self::Error> {
        let hdr_value = hdr_value.to_string();
        match HeaderValue::from_str(&hdr_value) {
            std::result::Result::Ok(value) => std::result::Result::Ok(value),
            std::result::Result::Err(e) => std::result::Result::Err(format!(
                r#"Invalid header value for ArenaLeaderboardResponse - value: {hdr_value} is invalid {e}"#
            )),
        }
    }
}
        ...
        match hdr_value.to_str() {
            std::result::Result::Ok(value) => {
                match <ArenaLeaderboardResponse as std::str::FromStr>::from_str(value) {
                    ...
                    std::result::Result::Err(err) => std::result::Result::Err(format!(
                        r#"Unable to convert header value '{value}' into ArenaLeaderboardResponse - {err}"#
                    )),
                }
            }
            std::result::Result::Err(e) => std::result::Result::Err(format!(
