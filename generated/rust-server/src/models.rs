    ...
    fn from_str(s: &str) -> std::result::Result<Self, Self::Err> {
        ...
        while key_result.is_some() {
            let val = match string_iter.next() {
                Some(x) => x,
                None => return std::result::Result::Err("Missing value while parsing ApiError".to_string())
            };
            ...
            if let Some(key) = key_result {
                ...
                match key {
                    ...
                    _ => return std::result::Result::Err("Unexpected key while parsing ApiError".to_string())
                }
            }
        ...
        std::result::Result::Ok(ApiError {
            code: intermediate_rep.code.into_iter().next().ok_or_else(|| "code missing in ApiError".to_string())?,
            message: intermediate_rep.message.into_iter().next().ok_or_else(|| "message missing in ApiError".to_string())?,
        })
    }
}
...
impl std::convert::TryFrom<header::IntoHeaderValue<ApiError>> for HeaderValue {
    ...
    fn try_from(hdr_value: header::IntoHeaderValue<ApiError>) -> std::result::Result<Self, Self::Error> {
        let hdr_value = hdr_value.to_string();
        match HeaderValue::from_str(&hdr_value) {
             std::result::Result::Ok(value) => std::result::Result::Ok(value),
             std::result::Result::Err(e) => std::result::Result::Err(format!(r#"Invalid header value for ApiError - value: {hdr_value} is invalid {e}"#))
        }
    }
}
        ...
        match hdr_value.to_str() {
             std::result::Result::Ok(value) => {
                    match <ApiError as std::str::FromStr>::from_str(value) {
                        ...
                        std::result::Result::Err(err) => std::result::Result::Err(format!(r#"Unable to convert header value '{value}' into ApiError - {err}"#))
                    }
             },
...
impl PlayerProfileSummary {
    #[allow(clippy::new_without_default, clippy::too_many_arguments)]
    pub fn new(player_name: String, tag_line: String, region: String, puuid: String, recent_placements: Vec<u8>, total_games: u32, win_rate: f64, last_updated: String, ) -> PlayerProfileSummary {
...
impl std::fmt::Display for PlayerProfileSummary {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let params: Vec<Option<String>> = vec![
            ...
            Some(self.recent_placements.iter().map(|x| x.to_string()).collect::<Vec<_>>().join(",")),
...
impl std::str::FromStr for PlayerProfileSummary {
    ...
    fn from_str(s: &str) -> std::result::Result<Self, Self::Err> {
        ...
        while key_result.is_some() {
            let val = match string_iter.next() {
                Some(x) => x,
                None => return std::result::Result::Err("Missing value while parsing PlayerProfileSummary".to_string())
            };
        ...
        std::result::Result::Ok(PlayerProfileSummary {
            player_name: intermediate_rep.player_name.into_iter().next().ok_or_else(|| "playerName missing in PlayerProfileSummary".to_string())?,
            tag_line: intermediate_rep.tag_line.into_iter().next().ok_or_else(|| "tagLine missing in PlayerProfileSummary".to_string())?,
            region: intermediate_rep.region.into_iter().next().ok_or_else(|| "region missing in PlayerProfileSummary".to_string())?,
            puuid: intermediate_rep.puuid.into_iter().next().ok_or_else(|| "puuid missing in PlayerProfileSummary".to_string())?,
            recent_placements: intermediate_rep.recent_placements.into_iter().next().ok_or_else(|| "recentPlacements missing in PlayerProfileSummary".to_string())?,
            total_games: intermediate_rep.total_games.into_iter().next().ok_or_else(|| "totalGames missing in PlayerProfileSummary".to_string())?,
            win_rate: intermediate_rep.win_rate.into_iter().next().ok_or_else(|| "winRate missing in PlayerProfileSummary".to_string())?,
            last_updated: intermediate_rep.last_updated.into_iter().next().ok_or_else(|| "lastUpdated missing in PlayerProfileSummary".to_string())?,
        })
    }
}
...
impl std::convert::TryFrom<header::IntoHeaderValue<PlayerProfileSummary>> for HeaderValue {
    ...
    fn try_from(hdr_value: header::IntoHeaderValue<PlayerProfileSummary>) -> std::result::Result<Self, Self::Error> {
        let hdr_value = hdr_value.to_string();
        match HeaderValue::from_str(&hdr_value) {
             std::result::Result::Ok(value) => std::result::Result::Ok(value),
             std::result::Result::Err(e) => std::result::Result::Err(format!(r#"Invalid header value for PlayerProfileSummary - value: {hdr_value} is invalid {e}"#))
        }
    }
}
        ...
        match hdr_value.to_str() {
             std::result::Result::Ok(value) => {
                    match <PlayerProfileSummary as std::str::FromStr>::from_str(value) {
                        std::result::Result::Ok(value) => std::result::Result::Ok(header::IntoHeaderValue(value)),
                        std::result::Result::Err(err) => std::result::Result::Err(format!(r#"Unable to convert header value '{value}' into PlayerProfileSummary - {err}"#))
                    }
             },
             std::result::Result::Err(e) => std::result::Result::Err(format!(r#"Unable to convert header: {hdr_value:?} to string: {e}"#))
        }
...
impl PlayerSearchResult {
    #[allow(clippy::new_without_default, clippy::too_many_arguments)]
    pub fn new(player_name: String, tag_line: String, region: String, puuid: String, last_updated: String, ) -> PlayerSearchResult {
        PlayerSearchResult {
 player_name,
 tag_line,
 region,
...
impl std::str::FromStr for PlayerSearchResult {
    ...
    fn from_str(s: &str) -> std::result::Result<Self, Self::Err> {
        ...
        std::result::Result::Ok(PlayerSearchResult {
            player_name: intermediate_rep.player_name.into_iter().next().ok_or_else(|| "playerName missing in PlayerSearchResult".to_string())?,
            tag_line: intermediate_rep.tag_line.into_iter().next().ok_or_else(|| "tagLine missing in PlayerSearchResult".to_string())?,
            region: intermediate_rep.region.into_iter().next().ok_or_else(|| "region missing in PlayerSearchResult".to_string())?,
            puuid: intermediate_rep.puuid.into_iter().next().ok_or_else(|| "puuid missing in PlayerSearchResult".to_string())?,
            last_updated: intermediate_rep.last_updated.into_iter().next().ok_or_else(|| "lastUpdated missing in PlayerSearchResult".to_string())?,
        })
    }
}
